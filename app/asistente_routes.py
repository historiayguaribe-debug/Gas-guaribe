...
async def consultar_ia(request: Request, pregunta: str = Form(...), db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    contexto = f" Datos de GAS GUARIBE: - Clientes: {total_clientes} - Cilindros disponibles: {cilindros_disp} - Pedidos pendientes: {pedidos_pend} - Ingresos: {ingresos:.2f} Bs. - Costos: {costos + gastos:.2f} Bs. - Utilidad: {utilidad:.2f} Bs. - Exoneraciones: {exoneraciones} "
    try:
        completion = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=[{"role": "system", "content": "Eres un asistente de GAS GUARIBE. Responde preguntas basadas en los datos proporcionados."}, {"role": "user", "content": f"{contexto}\nPregunta: {pregunta}"}], temperature=0.7, max_tokens=1024)

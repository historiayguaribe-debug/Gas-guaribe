...
@router.post("/crear")
async def crear_pedido_admin(request: Request, cliente_id: int = Form(...), tamano: str = Form(...), cantidad: int = Form(...), precio_unitario: float = Form(0.0), exonerado: bool = Form(False), proveedor_id: int = Form(None), db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user = await get_current_user(token)
    ...
    pedido = Pedido(cliente_id=cliente_id, tamano=tamano, cantidad=cantidad, precio_unitario=0 if exonerado else precio_unitario, exonerado=exonerado, proveedor_id=proveedor_id, estado="pendiente")
    db.add(pedido)
    db.commit()
    return RedirectResponse(url="/pedidos/", status_code=303)

@router.post("/actualizar_estado")
async def actualizar_estado(pedido_id: int = Form(...), nuevo_estado: str = Form(...), db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user = await get_current_user(token)
    verificar_rol(user, ["admin", "operativo"])
...
async def cliente_crear_pedido(request: Request, tamano: str = Form(...), cantidad: int = Form(...), db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user = await get_current_user(token)
    verificar_rol(user, ["cliente"])
    if cantidad <= 0:
        raise HTTPException(status_code=400, detail="La cantidad debe ser mayor a cero")
    ...
    if not cliente:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")
    pedido = Pedido(cliente_id=cliente.id, tamano=tamano, cantidad=cantidad, precio_unitario=0.0, exonerado=cliente.tipo == "Institución Exonerada", estado="pendiente")
    db.add(pedido)
    db.commit()

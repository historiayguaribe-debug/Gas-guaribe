...
@router.post("/crear")
async def crear_carga(request: Request, proveedor_id: int = Form(...), cantidad_P: int = Form(0), cantidad_M: int = Form(0), cantidad_G: int = Form(0), numero_factura: str = Form(...), gastos_logisticos: float = Form(0.0), db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user = await get_current_user(token)
    ...
    costo_total = cantidad_P * proveedor.precio_P + cantidad_M * proveedor.precio_M + cantidad_G * proveedor.precio_G
    carga = Carga(proveedor_id=proveedor_id, cantidad_P=cantidad_P, cantidad_M=cantidad_M, cantidad_G=cantidad_G, costo_total=costo_total, numero_factura=numero_factura, gastos_logisticos=gastos_logisticos, fecha=datetime.utcnow())
    db.add(carga)
    db.commit()
    for tam, cant in [("P", cantidad_P), ("M", cantidad_M), ("G", cantidad_G)]:
        for _ in range(cant):
            precio_venta = getattr(proveedor, f"precio_{tam}") * 1.3
            cil = Cilindro(codigo_qr=generar_codigo_qr(), tamano=tam, estado="disponible", proveedor_id=proveedor_id, costo_compra=getattr(proveedor, f"precio_{tam}"), precio_venta=precio_venta)
            db.add(cil)
    db.commit()

...
@router.post("/crear")
async def crear_venta(request: Request, cliente_id: int = Form(...), proveedor_id: int = Form(...), tamano: str = Form(...), cantidad: int = Form(...), precio_unitario: float = Form(...), exonerado: bool = Form(False), db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user = await get_current_user(token)
    ...
    venta = Venta(cliente_id=cliente_id, proveedor_id=proveedor_id, tamano=tamano, cantidad=cantidad, precio_unitario=precio_unitario if not exonerado else 0, exonerado=exonerado, fecha=datetime.utcnow())
    db.add(venta)
    db.commit()

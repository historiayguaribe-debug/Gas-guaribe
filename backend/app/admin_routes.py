...
# --- RUTAS HTML (SIN AUTENTICACIÓN) ---
@router.get("/dashboard", response_class=HTMLResponse)
async def admin_dashboard(request: Request):
    return templates.TemplateResponse("admin_dashboard.html", {"request": request})

@router.get("/cargas", response_class=HTMLResponse)
async def admin_cargas(request: Request, db: Session = Depends(get_db)):
...
async def admin_ventas(request: Request, db: Session = Depends(get_db)):
...
async def admin_asistente(request: Request):
    return templates.TemplateResponse("admin_chat.html", {"request": request})

# --- ENDPOINTS DE DATOS (SIN AUTENTICACIÓN) ---
...
async def get_estadisticas(db: Session = Depends(get_db)):
    ...
    ingresos_hoy = sum(float(p.monto_total) for p in db.query(Pedido).filter(Pedido.fecha_creacion >= hoy, Pedido.estado == "entregado").all())
    exonerados_hoy = db.query(DetallePedido).filter(DetallePedido.exonerado == True, DetallePedido.pedido.has(Pedido.fecha_creacion >= hoy)).count()
...
# --- ENDPOINTS POST (SIN AUTENTICACIÓN) ---
@router.post("/cargas")
async def registrar_carga(data: CargaRequest, db: Session = Depends(get_db)):
...
@router.post("/ventas")
async def registrar_venta(data: VentaRequest, db: Session = Depends(get_db)):
...
async def registrar_cliente(data: ClienteRequest, db: Session = Depends(get_db)):
